...
//	매개변수를 입력했을 때, 결과로 job를 가져오는 처리하는 기능 메서드 선언..
	...
	@Select("SELECT hisal FROM SALGRADE WHERE grade =#{grade}")
	...
	@Select("SELECT empno\r\n"
			+ "FROM emp\r\n"
			+ "WHERE deptno = #{deptno}")
	List<Integer> getEmpnos(@Param("deptno") int deptno);
	//ex2
	@Select("SELECT ename FROM emp where sal BETWEEN #{start} AND #{end}")
